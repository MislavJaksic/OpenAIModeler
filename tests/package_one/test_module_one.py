...
from openai_modeler.package_one.module_one import add
from openai_modeler.package_two import module_two
...
def calculator():
    # setup
    calculator = module_one.Calculator(
        "instance_value"
    )  # must be imported through module_one due to mocking!
    yield calculator
    # teardown
...
class TestMockCalculator:
    def test_multiply_mock(
        self, mock_calculator, calculator
    ):  # mock_calculator must come before the real Class!
